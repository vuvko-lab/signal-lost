...
- [ ] World generation via local tables
...
- [ ] Template system that generates entries from local tables per phase
...
1. **Entry pacing** — 8-15s per Vessel. With 3 Vessels, screen updates every ~3-5s. Too fast? Needs playtesting.
2. **Educational depth** — How much CS to include? Light flavor vs. actual explanations? Tooltips?
3. **Vessel cap** — How many simultaneous feeds before it's overwhelming? Probably 3-4 max.
4. **Audio licensing** — Need to verify specific Freesound tracks before bundling. Budget time for this.
